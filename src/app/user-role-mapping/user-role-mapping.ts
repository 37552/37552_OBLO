...
@Component({
  ...
  templateUrl: './user-role-mapping.html',
  styleUrl: './user-role-mapping.scss'
})
...
export class UserRoleMapping {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'user', header: 'User Name/Id', isVisible: true, isSortable: false },
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetUserRoleMapping|${query}`).subscribe({
        next: (res: any) => {     
          try {
            setTimeout(() => {
  ...
  showDialog(view: string, data: any) {
    ...
    if (view === 'add') {
      ...
      this.groupMasterForm1.patchValue({
        userType: userObj ? userObj.drpValue : '',
        userRole: roleObj ? roleObj.drpValue : '',
      });
    }
  ...
  onSubmit(event: any) {
    ...
    const userType = this.groupMasterForm1.get('userType')?.value || 0;
    const userRole = this.groupMasterForm1.get('userRole')?.value || 0;

    this.paramvaluedata = `userDrpId=${userType}|roleId=${userRole}`;
  ...
  deleteData() {
    try {
      this.isFormLoading = true;
      let query = `action=delete|Id=${this.selectedIndex.id}|roleId=0|uId=0|userId=${sessionStorage.getItem('userId')}`;
      this.userService.SubmitPostTypeData(`uspUpdateDeletetblUserRoleMapping`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
