...
@Component({
  ...
  templateUrl: './pre-bid-queries.html',
  styleUrl: './pre-bid-queries.scss'
})

export class PreBidQueries {
  isLoading = true;
  ...
  columns: TableColumn[] = [
    ...
    { key: 'queryReference', header: 'Query Reference', isVisible: true, isSortable: false },
    { key: 'queryDescription', header: 'Query Description', isVisible: true, isSortable: false },
    { key: 'departmentReply', header: 'Department Reply', isVisible: true, isSortable: false },
    { key: 'clarificationDate', header: 'ClarificationDate', isVisible: true, isSortable: false },
  ];
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      ...
      queryReference: ['', [Validators.required, noInvalidPipelineName()]],
      queryDeference: ['', [Validators.required, noInvalidPipelineName()]],
      departmentReply: ['',],
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetPreBidQueryDetails|${query}`).subscribe({
        next: (res: any) => { 
          try {
            setTimeout(() => {
  ...
  showDialog(view: string, data: any) {
    ...
    if (view === 'add') {
      ...
      this.groupMasterForm1.patchValue({
        ...
        queryDeference: data.queryDescription || '',
        departmentReply: data.departmentReply || '',
        clarificationDate: data.clarificationDate || '',
      });
    }
  ...
  onSubmit(event: any) {
    ...
    const organizationId = this.groupMasterForm1.get('organizationId')?.value;

    const queryReference = String(this.groupMasterForm1.get('queryReference')?.value || '').trim();
    const queryDeference = String(this.groupMasterForm1.get('queryDeference')?.value || '').trim();

    let departmentReply = this.groupMasterForm1.get('departmentReply')?.value;
    departmentReply = departmentReply ? String(departmentReply).trim() : '';

    const clarificationDate = String(this.groupMasterForm1.get('clarificationDate')?.value || '').trim();

    this.paramvaluedata =
      `vendorId=${organizationId}` +
      `|queryReference=${queryReference}` +
      `|queryDescription=${queryDeference}` +
      `|departmentReply=${departmentReply}` +
  ...
  deleteData() {
    try {
      ...
      this.userService.SubmitPostTypeData(`uspDeletePreBidQueries`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
