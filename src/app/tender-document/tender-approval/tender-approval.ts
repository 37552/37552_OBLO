...
@Component({
  ...
  templateUrl: './tender-approval.html',
  styleUrl: './tender-approval.scss'
})


export class TenderApproval {
  isLoading = true;
  ...
  columns: TableColumn[] = [
    ...
    { key: 'authorityName', header: 'Authority Name', isVisible: true, isSortable: false },
    { key: 'approvalStatus', header: 'Approval Status', isVisible: true, isSortable: false },
    { key: 'approvalAmount', header: 'Approval Amount', isVisible: true, isSortable: false },
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      ...
      statusId:['', [Validators.required, noInvalidPipelineName()]],
      approvaldate:['', Validators.required],
      approvalAmount: ['', [
        Validators.required,
        Validators.pattern(/^\d+(\.\d{1,2})?$/)
      ]],
      authorityName: ['', [Validators.required, noInvalidPipelineName()]],
    });
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetTenderApproveDetails|${query}`).subscribe({
        next: (res: any) => {
          try {
            setTimeout(() => {
  ...
  onSubmit(event: any) {
    ...
    const authorityName = String(this.groupMasterForm1.get('authorityName')?.value || '').trim();
    const approvalAmount = String(this.groupMasterForm1.get('approvalAmount')?.value || '').trim();
    const approvaldate = String(this.groupMasterForm1.get('approvaldate')?.value || '').trim();

    this.paramvaluedata = `tenderId=${tenderId}|approvalAmount=${approvalAmount}|authorityName=${authorityName}|approvaldate=${approvaldate}|approvalStatus=${statusId}`;
  ...
  deleteData() {
    try {
      ...
      this.userService.SubmitPostTypeData(`uspDeleteTenderApproval`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
