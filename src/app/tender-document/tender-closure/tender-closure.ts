...
@Component({
  ...
  templateUrl: './tender-closure.html',
  styleUrl: './tender-closure.scss'
})


export class TenderClosure {
  isLoading = true;
  ...
  columns: TableColumn[] = [
    ...
    { key: 'date', header: 'Closure Date', isVisible: true, isSortable: false },
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetTenderClosure|${query}`).subscribe({
        next: (res: any) => { 
          try {
            setTimeout(() => {
  ...
  onSubmit(event: any) {
    ...
    const tenderStatus = String(this.groupMasterForm1.get('tenderStatus')?.value || '').trim();
    const closuredate = String(this.groupMasterForm1.get('closuredate')?.value || '').trim();
    const reason = String(this.groupMasterForm1.get('reason')?.value || '').trim();

    this.paramvaluedata = `tenderId=${tenderId}|tenderStatusId=${tenderStatus}|date=${closuredate}|reason=${reason}`;
  ...
  submitcall() {
    ...
    if (this.postType === 'update') {
      ...
      SP = `uspUpdateTenderClosureDetails`;
    }
    else {
      query = `${this.paramvaluedata}|userId=${sessionStorage.getItem('userId')}`;
      SP = `uspPostTenderClosureDetails`;
    }
  ...
  deleteData() {
    try {
      ...
      this.userService.SubmitPostTypeData(`uspDeleteTenderClosureDetails`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
