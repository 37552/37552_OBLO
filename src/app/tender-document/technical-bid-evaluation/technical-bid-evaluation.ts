...
@Component({
  selector: 'app-technical-bid-evaluation',
  imports: [
    TableTemplate,
    CardModule,
    ButtonModule,
  ...
  templateUrl: './technical-bid-evaluation.html',
  styleUrl: './technical-bid-evaluation.scss'
})


export class TechnicalBidEvaluation {
  isLoading = true;
  ...
  columns: TableColumn[] = [
    ...
    { key: 'technicalScore', header: 'Technical Score', isVisible: true, isSortable: false },
    { key: 'remarks', header: 'Remarks', isVisible: true, isSortable: false },
    { key: 'eligiblity', header: 'Eligiblity', isVisible: true, isSortable: false },
  ];
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      ...
      eligiblityId: ['', Validators.required],
      technicalScore: ['', [
        Validators.required,
  ...
  GetTenderEligibleMaster() {
    this.userService.getQuestionPaper(`uspGetTenderEligibleMaster`).subscribe((res: any) => {
      this.tenderEligibleDrp = res['table']
    })
    setTimeout(() => {
      this.isLoading = false;
      this.cdr.detectChanges();
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetTechBidEvaluationForm|${query}`).subscribe({
        next: (res: any) => {
          console.log("uspGetTechBidEvaluationForm========",res);
  ...
  showDialog(view: string, data: any) {
    ...
    if (view === 'add') {
      ...
      this.groupMasterForm1.patchValue({
        ...
        eligiblityId: data.eligibilityId || '',
  ...
  onSubmit(event: any) {
    ...
    let eligiblityId = this.groupMasterForm1.get('eligiblityId')?.value;
    const technicalScore = String(this.groupMasterForm1.get('technicalScore')?.value || '').trim();
    const remarks = String(this.groupMasterForm1.get('remarks')?.value || '').trim();

    this.paramvaluedata = `tenderId=${tenderId}|eligiblityId=${eligiblityId}|technicalScore=${technicalScore}|remarks=${remarks}`;
  ...
  submitcall() {
    ...
    if (this.postType === 'update') {
      ...
      SP = `uspUpdateTechBidEvaluationForm`;
    }
    else {
      query = `${this.paramvaluedata}|userId=${sessionStorage.getItem('userId')}`;
      SP = `uspPostTechBidEvaluationForm`;
    }
  ...
  deleteData() {
    try {
      ...
      this.userService.SubmitPostTypeData(`uspDeleteTechBidEvaluationForm`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
