...
export class TaxRate {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'taxName', header: 'Tax Name', isVisible: true, isSortable: false },
    { key: 'taxvalue', header: 'Rate (%)', isVisible: true, isSortable: false },
  ];
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      taxName: ['', [Validators.required, noInvalidPipelineName()]],
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetTaxDetails|${query}`).subscribe({
        next: (res: any) => {
          try {
            setTimeout(() => {
  ...
  validateNumberInput(event: any, action: string) {
    ...
    if (action == '1') {
      this.groupMasterForm1.get('rate')?.setValue(value);
    }
  }
  ...
  onSubmit(event: any) {
    ...
    const taxName = String(this.groupMasterForm1.get('taxName')?.value || '').trim();
    const rate = String(this.groupMasterForm1.get('rate')?.value || '').trim();

    this.paramvaluedata = `taxDesc=${taxName}|taxValue=${rate}`;
  ...
  submitcall() {
    try {
      ...
      if (this.postType === 'update') {
        query = `${this.paramvaluedata}|action=update|taxId=${this.selectedIndex.id}|userId=${sessionStorage.getItem('userId')}`;
        SP = `uspUpdateDeleteSalesTaxDetails`;
      }
      else {
        query = `${this.paramvaluedata}|userId=${sessionStorage.getItem('userId')}`;
        SP = `uspPostTaxDetails`;
      }
  ...
  deleteData() {
    try {
      this.isFormLoading = true;
      let query = `action=Delete|taxId=${this.selectedIndex.id}|taxDesc=''|taxValue=0|userId=${sessionStorage.getItem('userId')}`;
      this.userService.SubmitPostTypeData(`uspUpdateDeleteSalesTaxDetails`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
