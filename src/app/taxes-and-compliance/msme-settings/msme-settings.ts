...
@Component({
  ...
  templateUrl: './msme-settings.html',
  styleUrl: './msme-settings.scss'
})

export class MsmeSettings {
  isLoading = true;
  ...
  columns: TableColumn[] = [
    ...
    { key: 'msmeCode', header: 'MSME Naming', isVisible: true, isSortable: false },
    { key: 'registrationType', header: 'MSME/Udyam Registration Type', isVisible: true, isSortable: false },
    { key: 'registrationNo', header: 'MSME/Udyam Registration Number', isVisible: true, isSortable: false },
  ];
 ...
 registrationTypedrp: any = []
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      ...
      registrationnumber: ['',[Validators.required, noInvalidPipelineName()]],
    });
  ...
  getRegistrationType() {
    ...
    this.userService.getQuestionPaper(`uspGetMSMERegistTypeMaster|${query}`).subscribe((res: any) => {
      this.registrationTypedrp = res['table']
    })
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetMsmeRegistrationDetails|${query}`).subscribe({
        next: (res: any) => {
          try {
            setTimeout(() => {
  ...
  showDialog(view: string, data: any) {
    ...
    if (view === 'add') {
      ...
      this.groupMasterForm1.patchValue({
        registrationtype: data?.registrationTypeId ? data?.registrationTypeId : '',
  ...
  onSubmit(event: any) {
    ...
    const taxexemption = this.groupMasterForm1.get('registrationtype')?.value || 0;
    const registrationnumber = String(this.groupMasterForm1.get('registrationnumber')?.value || '').trim();

    this.paramvaluedata = `msmeRegistTypeId=${taxexemption}|registrationNo=${registrationnumber}`;
  ...
  submitcall() {
    try {
      ...
      if (this.postType === 'update') {
        query = `${this.paramvaluedata}|actiom=update|msmeId=${this.selectedIndex.msmeId}|userId=${sessionStorage.getItem('userId')}`;
        SP = `uspUpdateDeleteMsmeRegistDetails`;
      }
      else {
        query = `${this.paramvaluedata}|userId=${sessionStorage.getItem('userId')}`;
        SP = `uspPostMsmeRegistrDetails`;
      }
  ...
  deleteData() {
    try {
      this.isFormLoading = true;
      let query = `action=delete|msmeId=${this.selectedIndex.msmeId}|registrationNo=''|msmeRegistTypeId=0|userId=${sessionStorage.getItem('userId')}`;
      this.userService.SubmitPostTypeData(`uspUpdateDeleteMsmeRegistDetails`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
