...
@Component({
  ...
  templateUrl: './tax-exemption.html',
  styleUrl: './tax-exemption.scss'
})
...
export class TaxExemption {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'exemption', header: 'Exemption', isVisible: true, isSortable: false },
    { key: 'exemptionType', header: 'Type', isVisible: true, isSortable: false },
    { key: 'reason', header: 'Reason', isVisible: true, isSortable: false },
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      ...
      reason: ['',[Validators.required, noInvalidPipelineName()]],
      description: ['',[Validators.required, noInvalidPipelineName()]],
      exemption: ['',[Validators.required, noInvalidPipelineName()]],
    });
  ...
  getExemptionDrp() {
    ...
    this.userService.getQuestionPaper(`uspGetTaxExemptionMaster|${query}`).subscribe((res: any) => {
      this.exemptionDrp = res['table']
    })
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetTaxExemptionDetails|${query}`).subscribe({
        next: (res: any) => {
          try {
            setTimeout(() => {
  ...
  showDialog(view: string, data: any) {
    ...
    if (view === 'add') {
      ...
      this.groupMasterForm1.patchValue({
        taxexemption: data?.exemptionTypeId ? data?.exemptionTypeId : '',
        reason: data.reason || '',
  ...
  onSubmit(event: any) {
    ...
    const taxexemption = this.groupMasterForm1.get('taxexemption')?.value || 0;
    const reason = String(this.groupMasterForm1.get('reason')?.value || '').trim();
    const exemption = String(this.groupMasterForm1.get('exemption')?.value || '').trim();
    const description = String(this.groupMasterForm1.get('description')?.value || '').trim();

    this.paramvaluedata = `exemption=${exemption}|reason=${reason}|description=${description}|exemptionId=${taxexemption}`;
  ...
  submitcall() {
    ...
    if (this.postType === 'update') {
      query = `${this.paramvaluedata}|actiom=update|id=${this.selectedIndex.exemptionId}|userId=${sessionStorage.getItem('userId')}`;
      SP = `uspUpdateDeleteTaxExemptionDetails`;
    }
    else {
      query = `${this.paramvaluedata}|userId=${sessionStorage.getItem('userId')}`;
      SP = `uspPostTaxExemptionDetails`;
    }
  ...
  deleteData() {
    try {
      this.isFormLoading = true;
      let query = `action=Delete|id=${this.selectedIndex.exemptionId}|description=''|Exemption=''|reason=''|ExemptionTypeId=0|userId=${sessionStorage.getItem('userId')}`;
      this.userService.SubmitPostTypeData(`uspUpdateDeleteTaxExemptionDetails`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
