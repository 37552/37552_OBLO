...
export class TerritoryForm {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'drpOption', header: 'Territory', isVisible: true, isSortable: false },
  ];
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      territory: ['',[Validators.required, noInvalidPipelineName()]],
    });
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetTerritory|${query}`).subscribe({
        next: (res: any) => {
          try {
            setTimeout(() => {
  ...
  onSubmit(event: any) {
    ...
    const territory = this.groupMasterForm1.get('territory')?.value?.trim() || '';

    if (this.postType === 'update') {
      this.paramvaluedata = `description=${territory}`;
    }
    else {
      this.paramvaluedata = `territory=${territory}`;
  ...
  submitcall() {
    ...
    if (this.postType === 'update') {
      query = `${this.paramvaluedata}|action=Update|territoryId=${this.selectedIndex.drpValue}|userId=${sessionStorage.getItem('userId')}`;
      SP = `uspUpdateDeleteTerritory`;
    }
    else {
  ...
  deleteData() {
    try {
      this.isFormLoading = true;
      let query = `action=Delete|description=''|territoryId=${this.selectedIndex.drpValue}|userId=${sessionStorage.getItem('userId')}`;
      this.userService.SubmitPostTypeData(`uspUpdateDeleteTerritory`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
