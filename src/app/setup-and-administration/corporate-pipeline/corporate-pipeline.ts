...
@Component({
  selector: 'app-corporate-pipeline',
  imports: [
    TableTemplate,
    CardModule,
    ButtonModule,
  ...
  templateUrl: './corporate-pipeline.html',
  styleUrl: './corporate-pipeline.scss'
})

export class CorporatePipeline {
  isLoading = true;
  ...
  columns: TableColumn[] = [
    ...
    { key: 'drpOption', header: 'Pipeline Name', isVisible: true, isSortable: false },
  ];
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      pipeline: ['', [Validators.required, noInvalidPipelineName()]]
    });
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetCorporateSales|${query}`).subscribe({
        next: (res: any) => {
          try {
  ...
  onSubmit(event: any) {
    ...
    const pipeline = this.groupMasterForm1.get('pipeline')?.value?.trim() || '';

    if (this.postType === 'update') {
      this.paramvaluedata = `description=${pipeline}`;
    }
    else {
      this.paramvaluedata = `Corporate=${pipeline}`;
  ...
  submitcall() {
    try {
      ...
      if (this.postType === 'update') {
        query = `${this.paramvaluedata}|action=Update|corporateId=${this.selectedIndex.drpValue}|userId=${sessionStorage.getItem('userId')}`;
        SP = `uspUpdateDeleteCorporateSales`;
      }
      else {
        query = `${this.paramvaluedata}|userID=${sessionStorage.getItem('userId')}`;
        SP = `uspPostCorporateSales`;
      }
  ...
  deleteData() {
    try {
      this.isFormLoading = true;
      let query = `action=Delete|description=''|corporateId=${this.selectedIndex.drpValue}|userId=${sessionStorage.getItem('userId')}`;
      this.userService.SubmitPostTypeData(`uspUpdateDeleteCorporateSales`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
