...
@Component({
  ...
  templateUrl: './custom-clearance.html',
  styleUrl: './custom-clearance.scss'
})
...
export class CustomClearance {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'serialNo', header: 'Custon Clearance ', isVisible: true, isSortable: false },
    { key: 'billOFEntry', header: 'Bill of Entry', isVisible: true, isSortable: false },
    { key: 'clearanceDate', header: 'Clearnce Date', isVisible: true, isSortable: false },
    { key: 'dutyAssessed', header: 'Duty Assessed (INR)', isVisible: true, isSortable: false },
    { key: 'paymentStatus', header: 'Paid Status', isVisible: true, isSortable: false },
    { key: 'eximShipment', header: 'Linked to Import Purchase Shipment', isVisible: true, isSortable: false },
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      billEntry: ['', [Validators.required, noInvalidPipelineName()]],
      clearanceDate: ['', Validators.required],
      dutyAssessedInr: ['', Validators.required],
      status: ['', Validators.required],
      linkedImport: ['', Validators.required],
  ...
  ngOnInit(): void {
    ...
    this.getEximShipmentMaster()
    this.getPaymentStatusMaster()
  ...
  getEximShipmentMaster() {
    this.userService.getQuestionPaper(`GetEximShipmentMaster`).subscribe((res: any) => {
      this.eximShipmentDrp = res['table'];
    })
  }

  getPaymentStatusMaster() {
    this.userService.getQuestionPaper(`GetpaymentStatusMaster`).subscribe((res: any) => {
      this.paymentStatus = res['table'];
    })
  }
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetCustomClearance|${query}`).subscribe({
        next: (res: any) => {
          try {
            setTimeout(() => {
  ...
  showDialog(view: string, data: any) {
    ...
    if (view === 'add') {
      ...
      this.groupMasterForm1.patchValue({
        billEntry: data.billOFEntry || '',
        clearanceDate: data.clearanceDate || '',
        dutyAssessedInr: data.dutyAssessed,
        status: data.paymentStatusId,       // FIXED
        linkedImport: data.eximShipmentId,  // FIXED
        attachment: data.uploadDocuments || '',
      });
      this.uploadedResumeUrl = `https://elocker.nobilitasinfotech.com/${data?.uploadDocuments}`;
    }
  ...
  onSubmit(event: any) {
    ...
    this.paramvaluedata =
      `billOfEntry=${form.billEntry}|` +
      `clearanceDate=${form.clearanceDate}|` +
      `dutyAssessed=${form.dutyAssessedInr}|` +
      `paidStatusId=${form.status}|` +
      `eximDocShipId=${form.linkedImport}|` +
      `uploadDocuments=${form.attachment}`;
  ...
  deleteData() {
    try {
      ...
      this.userService.SubmitPostTypeData(`uspDeleteCustomClearance`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
