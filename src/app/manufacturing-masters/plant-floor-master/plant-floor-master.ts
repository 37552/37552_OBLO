...
@Component({
  selector: 'app-plant-floor-master',
  imports: [
    TableTemplate,
    CardModule,
    ButtonModule,
  ...
  templateUrl: './plant-floor-master.html',
  styleUrl: './plant-floor-master.scss'
})

export class PlantFloorMaster {
  isLoading = true;
  ...
  columns: TableColumn[] = [
    ...
    { key: 'floorName',header:'Floor Name', isVisible: true, isSortable: false },
    { key: 'warehouse',header:'Warehouse Name', isVisible: true, isSortable: false },
    { key: 'jsonDetails', header: 'Workstation', isVisible: true, isSortable: false, isCustom: true },
  ];
  ...
  constructor(private fb: FormBuilder,
    ...
    this.groupMasterForm1 = this.fb.group({
      floorname: ['', [Validators.required, noInvalidPipelineName()]],
      workstationdata:  ['', Validators.required],
  ...
  getTableData(isTrue: boolean) {
    try {
      ...
      this.userService.getQuestionPaper(`uspGetPlantFloorMaster|${query}`).subscribe({
        next: (res: any) => {
          try {
            setTimeout(() => {
  ...
  showDialog(view: string, data: any) {    
    ...
    if (view === 'add') {
      ...
      try {
        const parsed = JSON.parse(data?.workStation || '[]');

        parsed.forEach((item: any) => {
          if (item.ct && Array.isArray(item.ct)) {
            item.ct.forEach((ctItem: any) => {
              customerTypes.push({
                drpOption: ctItem.StationDetail,
                drpValue: ctItem.stationId,
              });
            });
          } 
          else {
            customerTypes.push({
              drpOption: item.StationDetail,
              drpValue: item.stationId,
            });
          }
        });
      } 
  ...
  onSubmit(event: any) {
    ...
    const floorname = (this.groupMasterForm1.get('floorname')?.value ?? '').toString().trim();
    const warehouse = (this.groupMasterForm1.get('warehouse')?.value ?? '').toString().trim();
  
    // Make sure Workstation value is always an array
    const WorkstationValue = this.groupMasterForm1.get('workstationdata')?.value;
    const Workstation = Array.isArray(WorkstationValue)? WorkstationValue: WorkstationValue? [WorkstationValue] : [];
    ...
    // --- Prepare payload ---
    const verticalCust = Workstation.map((c: any) => ({
      stationId: c?.drpValue ?? c,
    }));
  
    const verticalCustStr = JSON.stringify(verticalCust);
    this.paramvaluedata = `floorName=${floorname}|wareHouseId=${warehouse}|workStation=${verticalCustStr}`;    
  ...
  deleteData() {
    try {
      this.isFormLoading = true;
      let query = `action=delete|id=${this.selectedIndex.id}|wareHouseId=0|floorName=''|workStation=''|appUserId=${sessionStorage.getItem('userId')}`;
      this.userService.SubmitPostTypeData(`uspUpdateDeleteFloorDetails`, query, 'header').subscribe({
        next: (datacom: any) => {
          this.isFormLoading = false;
          try {
            if (!datacom) return;
  ...
  showGrouplist(data: any) {
    ...
    try {
      const custArray = JSON.parse(data?.workStation || '[]');
      this.groupListArray = custArray.map((c: any) => ({
        stationid: c.stationId,
        stationDetail: c.StationDetail
      }));
    } catch (e) {
      console.error('Error parsing customerDetails', e);
      this.groupListArray = [];
