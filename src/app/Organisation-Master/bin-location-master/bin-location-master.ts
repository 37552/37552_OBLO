...
@Component({
  selector: 'app-bin-location-master',
  imports: [
    TableTemplate,
    CardModule,
    ButtonModule,
  ...
  templateUrl: './bin-location-master.html',
  styleUrl: './bin-location-master.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class BinLocationMaster {
  isLoading = true;
  ...
  data: any[] = [];
  binLocationForm: FormGroup; // Changed from warehouseForm to binLocationForm
  warehouses: any[] = []; // Changed from districts to warehouses
  columns: TableColumn[] = [
    { key: 'actions', header: '⚙️', isVisible: true, isCustom: true },
    { key: 'warehouseName', header: 'Warehouse', isVisible: true  },
    { key: 'aisle', header: 'Aisle', isVisible: true },
    { key: 'rack', header: 'Rack', isVisible: true  },
    { key: 'shelf', header: 'Shelf', isVisible: true  },
    { key: 'bin', header: 'Bin', isVisible: true },
    { key: 'fullLocation', header: 'Full Location', isVisible: true }
  ];
  pageNo = 1;
  pageSize = 5;
  searchText = '';
  ...
  get f() { return this.binLocationForm.controls }
  ...
  loadDropdowns() {
    // Load warehouses
    this.userService.getQuestionPaper(`uspGetFillDrpDown|table=tblWarehouseMaster`).subscribe({
      next: (res: any) => {
        this.warehouses = res['table'] || [];
        this.cdr.detectChanges();
      },
      error: (err) => {
  ...
  getTableData(isTrue: boolean) {
    ...
    this.userService.getQuestionPaper(`uspGetBinLocation|${query}`).subscribe({
      next: (res: any) => {
        try {
          this.data = res?.table1 || [];
          this.data = this.data.map(item => ({
            ...item,
            fullLocation: `${item.aisle}-${item.rack}-${item.shelf}-${item.bin}`,
      ...
      error: (err) => {
        ...
        this.message.add({ severity: 'error', summary: 'Error', detail: 'Failed to load bin locations.' });
        this.isLoading = false;
        this.cdr.detectChanges();
      }
    });
  ...
  showDialog(view: string, data: any) {
    ...
    if (view == 'add') {
      ...
      this.visible = true;
      this.postType = view;
      this.header = view === 'update' ? 'Update Bin Location' : 'View Bin Location';
      this.headerIcon = view === 'update' ? 'pi pi-pencil' : 'pi pi-eye';
      this.selectedIndex = data;
     
      if (view === 'view') {
        this.binLocationForm.disable();
      } else {
        this.binLocationForm.enable();
      }
      ...
      this.binLocationForm.patchValue(patchData);
      this.isFormLoading = false;
      this.cdr.detectChanges();
    }
  ...
  onSubmit(event: any) {
    if (!this.binLocationForm.valid) {
      this.binLocationForm.markAllAsTouched();
      this.cdr.detectChanges();
      return;
    }
    
    const rawValues = this.binLocationForm.getRawValue();
  ...
  submitcall() {
    ...
    let query = '';
    let SP = 'uspPostWarehouseBinLocation'; // Changed stored procedure
    ...
    this.userService.SubmitPostTypeData(SP, query, 'header').subscribe({
      next: (datacom: any) => {
        ...
        if (resultarray[1] === "success") {
          ...
          this.message.add({
            severity: 'success',
            summary: 'Success',
            detail: this.postType === 'update' ? 'Bin Location Updated Successfully.' : 'Bin Location Saved Successfully.',
          });
          this.onDrawerHide();
      ...
      error: (err) => {
        ...
        this.message.add({ severity: 'error', summary: 'Error', detail: 'Failed to save bin location data.' });
        this.isFormLoading = false;
        this.cdr.detectChanges();
      }
    });
  ...
  deleteData() {
    ...
    this.userService.SubmitPostTypeData(`uspDeleteWarehouseBinLocation`, query, 'header').subscribe({
      next: (datacom: any) => {
        this.isFormLoading = false;
        if (!datacom) return;
        const resultarray = datacom.split("-");
        if (resultarray[1] === "success") {
          this.getTableData(true);
          this.message.add({ severity: 'success', summary: 'Success', detail: 'Bin Location deleted successfully' });
          this.onDrawerHide();
        } else {
          this.message.add({ severity: 'warn', summary: 'Warn', detail: datacom, });
        }
      ...
      error: (err) => {
        ...
        this.message.add({ severity: 'error', summary: 'Error', detail: 'Failed to delete bin location.' });
        this.isFormLoading = false;
        this.cdr.detectChanges();
      }
    });
  ...
  isInvalid(field: string): boolean {
    const control = this.binLocationForm.get(field);
