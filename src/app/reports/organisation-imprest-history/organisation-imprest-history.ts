...
@Component({
  selector: 'app-organisation-imprest-history',
  standalone: true,
  ...
  templateUrl: './organisation-imprest-history.html',
  styleUrl: './organisation-imprest-history.scss'
})
export class OrganisationImprestHistory implements OnInit {
  constructor(
  ...
  breadcrumbItems: MenuItem[] = [
    ...
    { label: 'Organisation Imprest History', icon: 'pi pi-file-o', routerLink: '/organisation-imprest-history' },
  ];
  ...
  imprestTableColumns: TableColumn[] = [
    ...
    { key: 'empCode', header: 'Emp Code', isVisible: true, isSortable: false },
    { key: 'purpose', header: 'Purpose', isVisible: true, isSortable: false },
    { key: 'opBalance', header: 'Opening Balance', isVisible: true, isSortable: false },
    { key: 'dR_Amount', header: 'Debit', isVisible: true, isSortable: false },
    { key: 'cR_Amount', header: 'Credit', isVisible: true, isSortable: false },
    { key: 'closingBalance', header: 'Closing Balance', isVisible: true, isSortable: false },
  ];
  ...
  getDrpData() {
    this.userService
      .getQuestionPaper(
        `uspGetFillDrpDown|table=tblOrgMaster|appUserId=${sessionStorage.getItem('userId')}`,
      )
      .subscribe(
        (res: any) => {
  ...
  getFilterData(): void {
    ...
    const { organisationId, fromDate, toDate } = this.filterForm.value;
    ...
    if (!organisationId || organisationId === '0' || organisationId === 0) {
      this.messageService.add({
        severity: 'warn',
        summary: 'Validation',
    ...
    let query = `uspReportOrgImprest`;
    query += `|fromDate=${formattedFromDate}`;
    query += `|toDate=${formattedToDate}`;
    query += `|orgMasterId=${organisationId}`;
